  ...
  return (
    <div className={styles.container}>
     ...
     {playersList.map((player, key)=> 
      ...
      <div>Meczy:  {playersListInfo.find(e => e.player_id == player.player_id).lifetime.Matches} </div>
      </div>
     )}
     </div>
    </div>
...
export const getServerSideProps = async () => {
  ...
  "c0e46cee-52e9-49bd-89b4-64d21cfa358c" ]
    ...
    const playersListInfo = await Promise.all(
      players.map(getPlayerInfo)
    ); 
   ...
   async function getPlayer(item){
      ...
      async function getPlayerInfo(item){
        ...
        const resp = await fetch(`https://open.faceit.com/data/v4/players/${item}/stats/csgo`,{
                                  method: "GET",
                                  headers: {
                                    "Content-type": "application/json;charset=UTF-8",
                                    "Authorization" : "Bearer 7ba74f51-8955-4749-9d5b-18c5e1fd8e72"
    ...
    return {
        props: {playersList,playersListInfo}
          }
